...
Class ShiftController extends Controller{
	...
	public function edit(Shift $shift){
		...
		foreach($shift_details as $shift_detail){
			...
			$week['day_off'][$shift_detail->day] = $shift_detail->day_off;
		}
	...
	public function store(ShiftRequest $request, Shift $shift){
		...
		foreach(config('lists.week') as $day){
            ...
            $shift_detail->day_off = isset($day_off[$day]) ? 1 : 0;
        ...
        $response = $this->getSetupGuide($response,'shift');
		...
		foreach(config('lists.week') as $day){
			\App\ShiftDetail::whereShiftId($shift->id)
				...
				->update(['in_time' => ($week['in_time'][$day]) ? date('H:i:s',strtotime($week['in_time'][$day])) : '00:00:00',
					'out_time' => ($week['out_time'][$day]) ? date('H:i:s',strtotime($week['out_time'][$day])) : '00:00:00', 'overnight' => isset($overnight[$day]) ? 1 : 0, 'day_off' => isset($day_off[$day]) ? 1 : 0]);
		}
