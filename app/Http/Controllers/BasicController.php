...
trait BasicController {
    ...
    public function logActivity($data) {
        ...
        if(config('config.enable_activity_log') && $data['user_id'])
    ...
    public function attendanceLabel($attendance){
        if($attendance == 'P')
            return '<span class="badge badge-success" style="float:left">'.trans('messages.present').'</span>';
        ...
        elseif($attendance == 'H')
            return '<span class="badge badge-info">'.trans('messages.holiday').'/Off</span>';
        elseif($attendance == 'A')
    ...
    public function attendanceTag($attendance){
        ...
        elseif($attendance == 'late')
            return '<span style="float:left;" class="badge badge-danger"  data-toggle="tooltip" title="'.trans('messages.late').'">L</span> ';
    ...
    public function getAttendanceSummary($user,$from_date,$to_date){
        ...
        $leave_without_pay_approved = array();
        ...
        foreach($leaves as $leave){
            ...
            foreach($leave_date_approved as $date_approved){
                ...
                else if($leave->LeaveType->is_paid==0)
                    $leave_without_pay_approved[]=$date_approved;
                else
                    $leave_approved[] = $date_approved;

                     //array_push($leave_approved,$date_approved);
               // dd($leave->LeaveType->is_half_day);
            }
        }
        //;
        //dd($leave_approved);
        ...
        while($date <= $to_date){
            ...
            if(count($leave_without_pay_approved) && in_array($date,$leave_without_pay_approved))
                $attendance = 'LWP';
            else if(count($half_day_leave_approved) && in_array($date,$half_day_leave_approved))
                $attendance = 'HD';
            elseif(isset($in))
                $attendance = 'P';
