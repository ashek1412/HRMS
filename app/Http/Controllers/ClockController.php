...
Class ClockController extends Controller{
	...
	public function clockButton(Request $request){
		...
		$user_shift = getShift(date('Y-m-d', strtotime(' -1 day')),\Auth::user()->id);

		$clock_yesterday = Clock::where('user_id','=',\Auth::user()->id)
			->where('date','=',$yesterday)
			->where('clock_out','=',null)
			->count();


		if($user_shift->overnight==1 && $clock_yesterday ==1&& $clock==0)
		{
	...
	public function in(Request $request){
		...
		$clock->terminal_id = 999;
		$clock->shift_id=$user_shift->shift_id;
		$clock->save();
	...
	public function dailyAttendance(){
		...
		$user1 = \App\User::find($user_id);
		$usr_roll=$user1->roles->first()->name;

		if ($usr_roll == "br_incharge")
			$locations = \App\Location::whereIn('id',getLocation(null,1))->get()->pluck('name','id')->all();
		else
	    		 $locations = \App\Location::whereIn('id',getLocation())->get()->pluck('name','id')->all();

		if ($usr_roll == "dept_head")
			$departments =\App\Department::where('id',getUserDepartment($user_id))->get()->pluck('name','id')->all();
		else
			$departments =\App\Department::whereIsHidden(0)->get()->pluck('name','id')->all();
		...
		return view('clock.daily_attendance',compact('table_data','assets','menu','current_report','departments','locations'));
	}
	...
	public function dailyAttendanceLists(Request $request){
		...
		if($request->input('department_id'))
			$user_query->whereHas('profile.designation',function($query) use($request){
				$query->whereIn('department_id',$request->input('department_id'));
			});
		...
		//var_dump(json_encode($user_query));
		//dd($user_query);
		//var_dump($user_query->toSql());
        ...
        foreach($users as $user){
        	...
        	$user_shift = getShift($date,$user->id);
			//dd($user);
			//$user_shift = getClockShift($clocks->shift_id);
			...
			//$terminal=$clocks->where('date',$date)->where('user_id',$user->id)
			$records = $clocks->where('date',$date)->where('user_id',$user->id)->all();
			$leave = $leaves->where('user_id',$user->id)->first();
			$day_off=($user_shift->day_off)?1:0;

			if($leave && $leave->LeaveType->is_half_day)
				$attendance = 'HD';
			elseif(isset($in))
				$attendance = 'P';
			elseif($leave && !$leave->LeaveType->is_half_day)
				$attendance = 'L';
			elseif($holiday || $day_off)
				$attendance = 'H';
			elseif(!$holiday && !$day_off && $date <= date('Y-m-d'))
				$attendance = 'A';
			else
				$attendance = '';

			if($request->input('stat_code')=='A' &&  $attendance =='P')
				continue;
			else if($request->input('stat_code')!='A' &&  $attendance !='P')
				continue;
			...
			if($request->input('stat_code')=='O' &&  $late >0)
				continue;
			else if($request->input('stat_code')=='L' &&  $late ==0)
				continue;
			...
			$rows[] = array(
				$this->attendanceLabel($attendance).$tag,
				$user->full_name,
				...
				$terminal,
				$date,
				date("h:i a", strtotime($user_shift->in_time)),
	...
	public function dateWiseAttendanceLists(Request $request){
        ...
        while($date <= $to_date){
			...
			if(count($half_day_leave_approved) && in_array($date,$half_day_leave_approved))
				$attendance = 'HD';
			elseif(isset($in))
				$attendance = 'P';
...
//        ];
	...
	public function userWiseSummaryAttendance(){
        ...
        //$atdsumm=getAtdSummary(51,"2018-7-1" ,"2018-7-30" );
		...
		return view('clock.user_wise_summary_attendance',compact('table_data','assets','menu','current_report','departments','locations','accessible_users'));
	}
	...
	public function userWiseSummaryAttendanceLists(Request $request){
        ...
        if($request->input('user_id'))
            $user_query=\App\User::whereIn('id',$request->input('user_id'));
        else
            $user_query = \App\User::whereId($user_id);
        ...
        foreach($users as $user)
        {
        	$total_late = $total_early_leaving = $total_overtime = $total_working = $total_rest= $count_hpresent=0;
        	$tag_count = array();
	        $date = $from_date;
	        $attendance = [];
            $atdsumm=getAtdSummary($user->id,$from_date,$to_date);
            $frm=new DateTime($from_date);
            $to= new DateTime($to_date);
            $day_diff = $frm->diff($to)->days+1;
			...
			//	$total_rest += $rest;
				...
				if(in_array($date,$half_day_leave_approved))
					$attendance[] = 'HD';
				elseif(isset($in)) {
                    $attendance[] = 'P';
                }
            ...
            $count_present=$atdsumm['tot_present_cnt'];
			$total_overtime=$atdsumm['tot_ot'];
...
/*			$raw_graph_data[] = array(
	...
	public function clock(Request $request,$user_id,$date,$clock_id = null){
		...
		$clock->shift_id=$user_shift->shift_id;
		$clock->remarks = $request->input('remarks');
		$clock->updated_by=\Auth::user()->id;
        $clock->save();
	...
	public function dateWiseUserAttendance(){
		$user_id = \Auth::user()->id;
		$user1 = \App\User::find($user_id);
		$usr_roll=$user1->roles->first()->name;
		if ($usr_roll == "br_incharge")
		...
		$assets = ['datatable'];
		$menu = 'attendance,report';
		$current_report = 'date-wise-user-attendance';
		return view('clock.date_wise_user_attendance',compact('assets','menu','current_report','departments','locations','accessible_users'));
	}

	public function dateWiseUserAttendanceLists(Request $request)
	{

		//var_dump($request->input('department_id'));
		$fdt=$request->input('from_date');
		$tdt=$request->input('to_date');
		...
		//dd($loc);
		$param="&fromdate=".$fdt."&todate=".$tdt."&loc_id=".$loc."&dept_id=".$dept;

		$fname = "/rpt/user_wise_mon.rptdesign";
// Redirect browser
		$dest = "http://localhost:8080/birt/frameset?__report=";
