...
class UserController extends Controller
{
    ...
    protected $form = 'user-registration-form';
    ...
    public function index(){
        ...
        $menu = 'user';
        return view('user.index',compact('table_data','assets','menu','roles','designations','departments','locations'));
    }
    ...
    public function lists(Request $request){
      ...
      // $users = \App\User::all();
        ...
        if($request->has('department_id'))
            $query->whereHas('profile.designation',function($q) use ($request){
                $q->whereIn('department_id',$request->input('department_id'));
            });
       ...
       // testfunction($query);
        ...
        //$users = Cache::remember('test', 1, function () {
        //    return $query->get();
       // });
        ...
        $upd_sft=$upd_usr=$upd_usr_stat=0;
        if(Entrust::can('edit-user'))
            $upd_usr=1;
        if(Entrust::can('update-shift'))
            $upd_sft=1;
        if(Entrust::can('change-user-status'))
            $upd_usr_stat=1;
        ...
        foreach($users as $user){
           ...
           // foreach($user->roles as $role)
            ...
            $row = array(
                ...
                (($upd_usr||$upd_sft) ? '<a href="/user/'.$user->username.'" class="btn btn-xs btn-default"> <i class="fa fa-arrow-circle-right" data-toggle="tooltip" title="'.trans('messages.view').'"></i></a> ': '').
                (($user->status == 'active' && $upd_usr_stat) ? '<a href="#" class="btn btn-xs btn-default" data-ajax="1" data-extra="&user_id='.$user->id.'&status=ban" data-source="/change-user-status"> <i class="fa fa-ban" data-toggle="tooltip" title="'.trans('messages.ban').' '.trans('messages.user').'"></i></a>' : '').
                (($user->status == 'banned' && $upd_usr_stat) ? '<a href="#" class="btn btn-xs btn-default" data-ajax="1" data-extra="&user_id='.$user->id.'&status=active" data-source="/change-user-status"> <i class="fa fa-check" data-toggle="tooltip" title="'.trans('messages.activate').' '.trans('messages.user').'"></i></a>' : '').
                (($user->status == 'pending_approval' && $upd_usr_stat) ? '<a href="#" class="btn btn-xs btn-default" data-ajax="1" data-extra="&user_id='.$user->id.'&status=approve" data-source="/change-user-status"> <i class="fa fa-check" data-toggle="tooltip" title="'.trans('messages.approve').' '.trans('messages.user').'"></i></a>' : '')
               ...
               // (Entrust::can('delete-user') ? delete_form(['user.destroy',$user->id]) : '').'</div>',
                //(Entrust::can('update-shift') ? '<a href="/user-shift/'.$user->UserShift[0]->id.'/edit"  class="btn btn-xs btn-default" data-toggle="modal" data-target="#myModal" ><i class="fa fa-edit" data-toggle="tooltip" title="'.trans('messages.shift').'"></i></a>' : '').
                .'</div>',
    ...
    public function show($username){
        ...
        $qualification_score_types=\App\QualificationScoreType::all()->pluck('name','id')->all();
    ...
    public function profile($username = null){
        ...
        $qualification_skills = \App\QualificationSkill::all()->pluck('name','id')->all();
        $qualification_score_types=\App\QualificationScoreType::all()->pluck('name','id')->all();
        return view('user.profile',compact('user','user_relation','document_types','custom_social_field_values','custom_register_field_values','education_levels','qualification_languages','qualification_skills','qualification_score_types'));
    }
    ...
    public function userShift($username){
        $user = User::whereUsername($username)->first();
        //$user = \App\User::find($request->input('id'));

        return view('user.update_shift',compact('user'))->render();
    }
    ...
    public function shiftCalendarEvents(Request $request){
        ...
        $ushift=getDailyShift($first_day,  $last_day,\Auth::user()->id);
