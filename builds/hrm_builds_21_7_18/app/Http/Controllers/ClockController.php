...
Class ClockController extends Controller{
	...
	public function dailyAttendanceLists(Request $request){
		...
		//$usr_roll=$user1->roles->first()->name;
		$user_query = getAccessibleUser(\Auth::user()->id,0);
	...
	public function dateWiseAttendance(){
		...
		$accessible_users = getAccessibleUserList(\Auth::user()->id,0);
		$assets = ['datatable','graph'];
		$menu = 'attendance,report';
	...
	public function userWiseSummaryAttendanceLists(Request $request){
		...
		//$user_query = getAccessibleUser($user_id,0);
        $user_query=null;
        if($request->input('user_id'))
            $user_query=\App\User::whereId($request->input('user_id'));
        else
            $user_query = \App\User::with('userShift');
