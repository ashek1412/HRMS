...
class HomeController extends Controller
{
    ...
    public function calendarEvents(Request $request){
        ...
        foreach($ushift as $sft){
            ...
            $start = date('Y-m-d',strtotime($sft['fdate']));
            $end = date('Y-m-d',strtotime($sft['tdate']."T24:59:59"));
            if($sft['day_off']) {
                $title = "Day Off";
                $color = "red";
            }
            ...
            {
                $title = "IN - ".showTime($sft['in_time'])."\nOut - ".showTime($sft['out_time']);
                $color = $sft['clr'];
            }
            $events[] = array('title' => $title, 'start' => $start,'end' => $end,'allDay'=>false, 'color' => $color,'icon' => 'gift');
        }
        ...
        foreach($anniversaries as $anniversary){
            ...
            $title = trans('messages.anniversary');
            $desc = trans('messages.anniversary').' : '.$anniversary->User->full_name;
            $color = '#88b04b';
            $events[] = array('title' => $title,'description'=>$desc, 'start' => $start, 'color' => $color,'icon' => 'gift');
        }
        foreach($work_anniversaries as $work_anniversary){
            $start = date('Y').'-'.date('m-d',strtotime($work_anniversary->date_of_joining));
            $title = ((date('Y',strtotime($work_anniversary->date_of_joining) != date('Y')) ) ? (trans('messages.work').' '.trans('messages.anniversary')) : (""));
            $desc = ((date('Y',strtotime($work_anniversary->date_of_joining) != date('Y')) ) ? (trans('messages.work').' '.trans('messages.anniversary')) : (trans('messages.new').' '.trans('messages.joining')) ).' : '.$work_anniversary->User->full_name;

            $color = '#6dc066';
            $events[] = array('title' => $title, 'description'=>$desc,'start' => $start, 'color' => $color,'icon' => 'briefcase');
        }
        foreach($todos as $todo){
            $start = $todo->date;
            $title = trans('messages.to_do').' : '.$todo->title;
            $desc = trans('messages.to_do').' : '.$todo->title.' '.$todo->description;
            $color = '#ff0000';
            $url = '/todo/'.$todo->id.'/edit';
            $events[] = array('title' => $title, 'description'=>$desc, 'start' => $start, 'color' => $color, 'url' => $url,'icon' => 'list-ul');
        }
        foreach($holidays as $holiday){
            $start = $holiday->date;
            $title = trans('messages.holiday');
            $desc = trans('messages.holiday').' : '.$holiday->description;
            $color = '#133edb';
            $events[] = array('title' => $title,'description'=>$desc, 'start' => $start, 'color' => $color,'icon' => 'fighter-jet');
        }
...
/*        foreach($tasks as $task){
    ...
    public function shiftCalendarEvents(Request $request){
        ...
        $ushift=getDailyShift($first_day,  $last_day,$request->input('usrid'));
        ...
        foreach($ushift as $sft){
            ...
            $events[] = array('title' => $title, 'start' => $start,'end' => $end,'allDay'=>false, 'color' => $color,'icon' => '');
        }

        //foreach($ushift as $sft){

          //  $start = date('Y').'-'.date('m-d',strtotime($sft['date']));
          //  $title = "Out - ".showTime($sft['out_time']);
         //   $color = '#daa520';
