...
Class LeaveController extends Controller{
	...
	public function index(Leave $leave){
		...
		$dept_id=getUserDepartment(\Auth::user()->id,1);
		$dept_usr=getDepartmentUsers($dept_id);

		//dd($user1->getNameWithDesignationAndLocationAttribute());
		if(Entrust::can('create-leave')) {
			if ($usr_roll == "admin" || $usr_roll == "hr_admin")
				$accessible_users = getAccessibleUserList(\Auth::user()->id, 1);
			else if ($usr_roll == "br_incharge") {
				$accessible_users = getLocationUsers();
				array_push($accessible_users, $user1->getNameWithDesignationAndLocationAttribute());
			}
			else
			{
				$accessible_users = $dept_usr;
				array_push($accessible_users, $user1->getNameWithDesignationAndLocationAttribute());
			}
			//dd($accessible_users);
		}
		...
		{
			$accessible_users[\Auth::user()->id]=$user1->getNameWithDesignationAndLocationAttribute();
		}
		...
		$menu = 'leave';
		return view('leave.index',compact('table_data','assets','menu','leave_types','accessible_users','dept_usr'));
	}
	...
	public function lists(Request $request){
        ...
        foreach($leaves as $leave){
        	...
        	if($leave->status == 'approved')
				...
				//$date_approved=$leave->date_approved;
        	else
        		$date_approved = '';
	...
	public function show($uuid){
		...
		getUsersDetail($leave->alt_usr_id);
	...
	public function store(LeaveRequest $request, Leave $leave){
		...
		//if(strlen($request->input('usr_id'))>0)
		//	return response()->json(['message' => trans('messages.absent'), 'status' => 'error']);
	    ...
	    $leave->uuid = getUuid();
	    $leave->status = 'pending';
		$leave->user_id = $request->input('usr_id') ;
		$leave->req_by=\Auth::user()->id;
		$leave->save();
