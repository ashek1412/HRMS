				...
				<tr>
					...
					<td>
						@if($leave_status_detail->status == 'approved')
							@if(count(explode(',',$leave_status_detail->date_approved)) == dateDiff($leave->from_date,$leave->to_date))
								{{trans('messages.all').' '.trans('messages.date')}}
							@else
								<ol>
									@foreach(explode(',',$leave_status_detail->date_approved) as $date_approved)
